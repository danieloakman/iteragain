import { describe, it } from 'bun:test';
import { equal, throws } from './internal/test-utils';
import { iter } from '.';

describe('iter', () => {
  it('should map values from an array', async function () {
    equal(
      iter([1, 2, 3])
        .map(n => n * n)
        .toArray(),
      [1, 4, 9],
    );
  });

  it('should map entries from an object', async function () {
    equal(
      iter({ a: 1, b: 2, c: 3 })
        .map(v => [v[0], v[1] * 2])
        .toArray(),
      [
        ['a', 2],
        ['b', 4],
        ['c', 6],
      ],
    );
  });

  it('should throw when converting an invalid iterator to array', async function () {
    // Probably won't end up handling this, as it would slow down `iter` a bit.
    throws(() => iter({ next() {} }).toArray());
  });

  it('should traverse nested objects in post-order DFS', async function () {
    /** @returns post-order-DFS traversal of `obj` by using the `reviver` cb of `JSON.parse`. */
    function jsonStrParse(obj: any): [string, any][] {
      const result: any[] = [];
      JSON.parse(JSON.stringify(obj), (k, v) => (result.push([k, v]), v));
      return result.slice(0, result.length - 1);
    }
    const obj = { a: 1, b: { c: 2, d: { e: 3 } }, f: 4 };
    equal(
      [...iter(obj).map(v => v[0])],
      iter(jsonStrParse(obj))
        .map(v => v[0])
        .toArray(),
    );
  });
});
